...
const getLogement = async () => {
    ...
    try {
        const response = await fetch(`http://localhost:9003/logements/${logementId}`)
        logementToDisplay = await response.json();

        console.log(logementToDisplay)
    } catch (error) {
        console.log(error);
    }
    displayLogements()
