...
const getLogements = async () => {
    ...
    let logements = []
    try {
        const query = await db.query('SELECT logement.id, logement.titre, logement.description, logement.nombre_pieces, logement.nombre_chambres, logement.surface_interieure, logement.surface_exterieure, logement.animaux, logement.id_photo, photo.url, logement.id_centre, centre_vacances.nom as nom_centre_vacances FROM logement JOIN centre_vacances ON centre_vacances.id = id_centre JOIN photo ON photo.id = logement.id_photo')
...
const getLogement = async (paramsId) => {
    let logement = []
    try {
        const query = await db.query('SELECT logement.id, logement.titre, logement.description, logement.nombre_pieces, logement.nombre_chambres, logement.surface_interieure, logement.surface_exterieure, logement.animaux, logement.id_photo, photo.url, logement.id_centre, centre_vacances.nom as nom_centre_vacances FROM logement JOIN centre_vacances ON centre_vacances.id = id_centre JOIN photo ON photo.id = logement.id_photo WHERE logement.id = $1', [paramsId])
        logement = query.rows;
...
const updateLogement = async (paramsId, logement) => {
    ...
    try {
        ...
        // await db.query('UPDATE logement SET titre = $1, description = $2 WHERE id = $2, [titre, description, nombrePieces,... paramsId]');
