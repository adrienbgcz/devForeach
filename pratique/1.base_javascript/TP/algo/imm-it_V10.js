...
function rechercheParEquipement(mesLogements, listeEquipements) {
    ...
    mesLogements.forEach(logement => {
        logementIsOK = false;
        let idEquipementsCourants = logement.equipements.map(element => {
           return element.equipement.id
        })
...
function isExist(tableau, choix) {
   ...
   exist = tableau.find(elementCourant => {
       return elementCourant == choix
    })

    return exist
}

// find retourne true ou undefined si la condition n'est pas respectée, si condition ok, find retourne true et le return de fin = 1
