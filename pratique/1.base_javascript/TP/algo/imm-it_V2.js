...
function rechercheParEquipement(mesLogements, listeEquipements) {
    // afficher les equipements à choisir
    afficherEquipements(listeEquipements)

    // demander à l'utilisateur de choisir des equipements parmi cette liste
    const results = choixEquipements(listeEquipements)
    console.log(results)
...
function choixEquipements(listeEquipements){
    ...
    while (!choixIsOk) {
        choix = 0;
        console.log('\n\nChoisissez un equipement dans la liste : \n')
    ...
    const equipementsChoisis = listeEquipements.filter(equipement => choix == equipement.id)
    
    return equipementsChoisis
