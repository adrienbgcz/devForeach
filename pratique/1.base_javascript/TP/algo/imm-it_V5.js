...
function rechercheParEquipement(mesLogements, listeEquipements) {
    ...
    // demander à l'utilisateur de choisir des equipements parmi cette liste, retourner un tableau
    const equipementsChoisis = choixEquipements(listeEquipements)
    console.log(equipementsChoisis)

    // rechercher quels logements contiennent TOUS les équipements recherchés
...
function choixEquipements(listeEquipements) {
    ...
    while (!quitter) {
        ...
        else if (choix < 0 || choix > listeEquipements.length) {
        ...
        console.log(`\nVos choix actuels : ${listeChoixEquipements}`);
    }
    return listeChoixEquipements
...
function classementParNote(mesLogements) { // TODO : ajouter les réservations dans l'affichage du logement
    ...
    mesLogements.forEach(logement => {
        ...
        const noteMoyenne = somme / logement.commentaires.length;
...
// et la date de départ supérieure à la date d'arrivée
    ...
    while (!dateDepartIsOk) {
        ...
        if (dateDepartToEn > dateArrivee) {
            dateDepart = dateDepartToEn;
            dateDepartIsOk = true;
        } else {
...
function rechercheParDate(mesLogements) {
    ...
    const choix1 = readlineSync.question('\nRenseignez une date d\'arrivée au format AAAA-MM-JJ : \n')
    const choix2 = readlineSync.question('\nRenseignez votre date de départ au format AAAA-MM-JJ : \n')

    const choixArrivee = new Date(choix1);
    const choixDepart = new Date(choix2);


    mesLogements.forEach(logement => {
        let isFree = false;
        if (logement.reservations == 0) {
            isFree = true;
        } else {
            logement.reservations.forEach(reservation => {
                if (choixArrivee < reservation.date_arrivee && choixDepart < reservation.date_arrivee) {
                    isFree = true;
                } else if (choixArrivee > reservation.date_depart && choixDepart > reservation.date_depart) {
                    isFree = true;
                }
            });
        }
    ...
    logementsCorrespondants.forEach(logement => console.log(`- ${logement.titre}`))
        ...
        console.log('Aucun logement n\'est disponible aux dates choisies')
    }
}
