...
function listerAuteursBibliothequeSansDoublon(maBibliotheque) {
    ...
    for (let i = 0; i < maBibliotheque.length; i++) {
        ...
        for (let j = 0; j < auteurs.length; j++) {
            ...
            let exist = autorExist(listeAuteurs, auteur);
...
function bibliothequeParAuteur(choix, maBibliotheque) {
    let ouvrageAuteur = [];
...
function listerLivresParAuteur(maBibliotheque) {
    ...
    const bibliothequeAuteur = bibliothequeParAuteur(choixAuteur, maBibliotheque);
...
function affichageMotsCles(mesMotsCles) {
...
function listerLivresParMotsCles(maBibliotheque) {
    ...
    affichageMotsCles(motsclesDisponibles);


    const choixMotCle = readlineSync.questionInt('\nVeuillez choisir un mot-clé : \n')

    const ouvrageMotsCles = bibliothequeParMotsCles(maBibliotheque, choixMotCle)


    listerTitresBibliotheque(ouvrageMotsCles)
...
function bibliothequeParMotsCles(maBibliotheque, numMotCle) {
    let ouvrageMotsCles = [];
...
//listerLivresParAuteur(bibliotheque1);
